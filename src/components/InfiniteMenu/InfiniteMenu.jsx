...
void main() {
    ...
    float glassEdge = smoothstep(0.78, 0.98, length(vUvs - 0.5) * 2.0);
    texColor.rgb = mix(texColor.rgb, vec3(1.0, 0.78, 0.48), glassEdge * 0.55);
    texColor.a *= vAlpha;
    outColor = texColor;
}
`;
...
class InfiniteGridMenu {
  ...
  start() {
    ...
    this.mediaSources.forEach(source => {
      if (source instanceof HTMLVideoElement) source.play().catch(() => {});
    });
    this.rafId = requestAnimationFrame(t => this.run(t));
  }
  ...
  #initTexture() {
    ...
    this.atlasCanvas.width = this.atlasSize * this.atlasCellSize;
    this.atlasCanvas.height = this.atlasSize * this.atlasCellSize;
    this.atlasContext.fillStyle = '#080808';
    this.atlasContext.fillRect(0, 0, this.atlasCanvas.width, this.atlasCanvas.height);

    this.mediaSources = this.items.map(item => {
      if (item.video) {
  ...
  #updateTexture() {
    if (this.#time - this.lastTextureUpdate < 66 || !this.atlasContext) return;
    this.lastTextureUpdate = this.#time;

    const ctx = this.atlasContext;
    const cell = this.atlasCellSize;
    ...
    this.mediaSources.forEach((source, index) => {
      ...
      ctx.drawImage(source, sx, sy, side, side, x, y, cell, cell);
    });
...
export default function InfiniteMenu({ items = [], scale = 1.0 }) {
  ...
  useEffect(() => {
    ...
    if (canvas) {
      ...
      observer = new IntersectionObserver(([entry]) => {
        if (entry.isIntersecting) sketch.start();
        else sketch.stop();
      }, { threshold: 0.08 });
  ...
  return (
    <div className="infinite-menu-shell">
      <canvas className="infinite-grid-menu-canvas" ref={canvasRef} aria-label="可拖拽的作品视频菜单" />

      {activeItem && (
        <>
          <h2 className={`face-title glass-menu-label ${isMoving ? 'inactive' : 'active'}`}>{activeItem.title}</h2>

          <p className={`face-description glass-menu-label ${isMoving ? 'inactive' : 'active'}`}>{activeItem.description}</p>

          <button
            type="button"
            aria-label={`播放 ${activeItem.title}`}
      ...
      {previewItem && (
        <div className="work-preview-overlay" onClick={() => setPreviewItem(null)}>
          <div className="work-preview-dialog" onClick={event => event.stopPropagation()}>
            <button type="button" className="work-preview-close" aria-label="关闭视频" onClick={() => setPreviewItem(null)}>
              &times;
            </button>
            <video src={previewItem.video} autoPlay muted loop playsInline controls />
